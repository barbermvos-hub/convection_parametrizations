...
data = np.load("data/Gaspar/forcing/bifurcation_equilibria_gamma-0.2.npz")
S.gamma = data["gamma"]
S.temp  = data["temp"][:]
S.sal   = data["sal"][:]
...
anim.save(f'data/Gaspar/forcing/Gaspar_cooling_seasonal_gamma{S.gamma}.gif')
plt.show()
...
plt.xlabel('Time [hours]')
plt.ylabel('Surface buoyancy flux [W/m^2]') 
plt.savefig(f'data/Gaspar/forcing/Gaspar_surface_buoyancy_flux_stochastic_winter_gamma{S.gamma}.png')
